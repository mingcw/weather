...
以上方法的第二个参数为返回值类型，可选 `json` 与 `xml`，默认 `json`：
...
- $city - 城市名或编码。比如：“深圳”或“440300”；
- $format - 输出的数据格式。默认为 “json” 格式，当设置为 “xml” 时，输出为 XML 格式的数据。
...
可以用两种方式来获取 `Mingcw\Weather\Weather` 实例：
